$(function() {
    ...
    $("#change-session").on("change", function(e) {
        //console.log( $("#change-session").val() );
...
function loadSVG(session) {
    ...
    d3.csv("data/s-"+session+"-all-t-info.csv", function(error, data) {  
      ...
      function toggleParties(options) {
        ...
        circles.filter(function(d) {
            ...
            if(d.party == options) {
                ...
                //partyTip
                //$("#tt-two").html('Party: <strong>' + d.party + '</strong> <ul> <li> Attendance >= 75%: ' + dist + '/' + count + ' MPs </li> <li> 65% <= Attendance < 75% ' + distLess + '/' + count + '</li> <li> 65% <= Attendance < 75% ' + distThird + '/' + count + '</li> </ul>' );
            }
            return d.party != options;
        })
        .style("opacity", "0.1")
